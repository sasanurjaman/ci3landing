    ...
    $(document).ready(function()
    {
    	$("button[name=btnbaru]").click(function(e) 
        {
            $.ajax({
                type: "post",
                dataType: "html",
                url: "<?php echo base_url();?>transactions/addnews",
                beforeSend: function() {
        ...
        <?php if (getAccess($this->session->userdata('SESS_USER_ID'), 'T1', 'new') == 'N'):?>
        ...
        <div class="col-md-12">
            <div class="box box-<?php echo $this->config->item('site_theme');?>">
                <div class="box-header with-border">
                    <h3 class="box-title"><i class="fa fa-newspaper-o"></i> <?php echo 'DATA '.$caption;?></h3>
                    <div class="box-tools">
                        <form class="search-bar" method="post" action="<?php echo base_url()."transactions/datanews";?>">
                                ...
                                <?php foreach ($datas as $row):?>
                                    <tr>
                                        ...
                                        <td class="center">
                                            <div class="btn-group">
                                                <?php if ($row->status == 'Y'):?>
                                                    <a class="btn btn-success btn-sm" href="<?php echo base_url()."transactions/activenews/".$search."/".$page."/".$row->id;?>" title="Aktif Data"><i class="fa fa-check"></i></a>
                                                <?php else:?>
                                                    <a class="btn btn-danger btn-sm" href="<?php echo base_url()."transactions/activenews/".$search."/".$page."/".$row->id;?>" title="Aktif Data"><i class="fa fa-close"></i></a>
                                                <?php endif;?>
                                            </div>
                                        </td>
                                        <td class="center">
                                            <div class="btn-group">
                                                <?php if (getAccess($this->session->userdata('SESS_USER_ID'), 'T1', 'edit') == 'Y'):?>
                                                    <a class="btn btn-info btn-sm" href="<?php echo base_url()."transactions/editnews/".$search."/".$page."/".$row->id;?>" title="Ubah Data"><i class="fa fa-edit"></i></a>
                                                <?php else:?>
                                                    <a class="btn btn-info btn-sm disabled" href="#" title="Edit Data"><i class="fa fa-edit"></i></a>
                                                <?php endif;?>
                                                <?php if (getAccess($this->session->userdata('SESS_USER_ID'), 'T1', 'delete') == 'Y'):?>
                                                    <a class="btn btn-danger btn-sm" href="<?php echo base_url()."transactions/delnews/".$search."/".$page."/".$row->id;?>" title="Hapus Data"><i class="fa fa-trash-o"></i></a>
                                                <?php else:?>
